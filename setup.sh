...
# Show first 4 + stars + last 4 so the user can verify the paste without revealing the full key
mask_key() {
  local val="$1"
  local len=${#val}
  if [ $len -eq 0 ]; then
